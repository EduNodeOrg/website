...
export function AlertDialog() {
  ...
  const handleConfirm = (e) => {
    setOpen(false);
    window.location.href = '/courses/111/';


  };
  const handleButtonClick = () => {
    const id11 ='6841abca38a24bd982c9d70a';
    window.location.href = `/courseDetails/${id11}`;
  };
...
export default function Course10() {
  ...
  const courseId = '6841abca38a24bd982c9d70a';
  ...
  useEffect(() => {
    const getCourseAverageRate = async () => {
      try {
        ...
        console.log('Rating API response:', response.data); // Add this line
        setRating(response.data.averageRating);
      } catch (error) {
        console.error('Error fetching rating:', error);
        setRating(0); // Set a default rating if API fails
      }
    };
          ...
          <Grid item xs={12} sm container>
            <Grid item xs container direction="column" spacing={2}>
              <Grid item xs>
                ...
                Iniciativa educativa para empoderar mujeres en Web3 con sesiones sobre Blockchain, DeFi, NFTs, DAOs y Gobernanza.
                </Typography>
